...
class Encoder(nn.Module):
    ...
    def forward(self, adj, x, idx):
        ...
        out = torch.zeros(
            torch.max(idx) + 1, x.size(1), device=x.device, requires_grad=False
        )
...
class VariationalAutoEncoder(nn.Module):
    ...
    def loss_function(self, adj, x, idx, y, beta=0.05):
        ...
        recon = F.binary_cross_entropy_with_logits(
            adj[:, triu_idx[0, :], triu_idx[1, :]],
            y[:, triu_idx[0, :], triu_idx[1, :]],
            reduction="sum",
            pos_weight=torch.tensor(1.0 / 0.4),
        )

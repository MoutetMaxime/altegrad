...
def normalize_adjacency(A):
    ...
    Atilde = A + sp.eye(A.shape[0])
    Dtilde = np.array(Atilde.sum(axis=1)).flatten()
    Dtilde_inv_sqrt = sp.diags(1.0 / np.sqrt(Dtilde))
    A_normalized = Dtilde_inv_sqrt @ Atilde @ Dtilde_inv_sqrt
    return A_normalized
